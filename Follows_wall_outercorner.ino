...
// Distance Thresholds
const int STOP_DISTANCE = 15;  // Stop if obstacle ahead (cm)
const int TARGET_LEFT = 10;    // Desired distance from the wall (cm)
const int TOLERANCE = 2;       // Acceptable error in wall distance (cm)

// PID Constants (for proportional control)
const float Kp = 5.0;          // Adjusts how aggressively the robot corrects

// Motor Speeds
const int BASE_SPEED = 130;    // Normal speed
const int TURN_SPEED = 90;     // Slower speed for turns
...
void turnLeft() {
  ...
  digitalWrite(RIGHT_FORWARD, HIGH); // Right motor keeps moving
  digitalWrite(RIGHT_BACKWARD, LOW);
}
...
void turnRight() {
  ...
  digitalWrite(LEFT_FORWARD, HIGH);  // Left motor keeps moving
  digitalWrite(LEFT_BACKWARD, LOW);
}
...
void loop() {
  ...
  if (frontDist > 0 && frontDist < STOP_DISTANCE) {  
    Serial.println("Obstacle detected! Moving back and turning left.");
    digitalWrite(LED_FRONT, HIGH);
    
    moveBackward();
    delay(700);  // Move back slightly
    
    turnLeft();
    delay(600);  // Small left turn
